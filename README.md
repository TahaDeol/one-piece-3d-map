...
**Why geodetic, not Mercator.** The first version tiled the map with GDAL's `mercator` profile — the same projection Google Maps uses. Mercator is mathematically undefined past ±85.05° latitude, so squeezing a full ±90° source image through it stretched and warped a wide band near the poles. Since this is a fan map, not a navigational chart, there's no reason to pay that penalty: the tiles are generated with GDAL's `geodetic` (plate carrée) profile instead, paired with Cesium's `GeographicTilingScheme`, which represents the full latitude range linearly — eliminating that band of distortion, and incidentally about half the redundant tile coverage Mercator's square-grid framing would have produced. Right at the true poles, every meridian still converges to a single point and shows some streaking if you rotate all the way there — that's an inherent property of any lat/lon-tiled globe, not something geodetic (or any tiling scheme) removes.

A few pole-specific fixes were tried and reverted rather than shipped: clipping the imagery layer's `rectangle` near the poles (Cesium still stretched the nearest tile row across the degenerate mesh regardless — checked in the browser's dev tools and found tiles were still being fetched all the way to the true edge); a flat-color disc mask over the pole (rejected — it hides real geography, and the Red Line converging there is supposed to be visible); blurring the source image at the poles; and a custom-geometry radial "cap" built from real map pixels (looked correct at first glance but didn't hold up under real interactive use). None of these actually fix the underlying issue — the source image only has data up to ±90°, and any lat/lon tiling scheme forces that flat rectangle to a single point at the pole, so some convergence artifact is unavoidable without fabricating pixels that were never in the original map.
